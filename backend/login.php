<?php
// ============================================================
//  SEGURIDAD — Si ya está autenticado, redirigir
// ============================================================
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Protección básica contra fuerza bruta (rate limiting por sesión)
    if (!isset($_SESSION['intentos'])) $_SESSION['intentos'] = 0;
    if (!isset($_SESSION['ultimo_intento'])) $_SESSION['ultimo_intento'] = 0;

    $tiempo_espera = 30; // segundos de bloqueo tras 5 intentos
    $max_intentos  = 5;

    if ($_SESSION['intentos'] >= $max_intentos) {
        $segundos_restantes = $tiempo_espera - (time() - $_SESSION['ultimo_intento']);
        if ($segundos_restantes > 0) {
            $error = "Demasiados intentos. Espera {$segundos_restantes} segundos.";
        } else {
            // Resetear contador tras el tiempo de espera
            $_SESSION['intentos'] = 0;
        }
    }
    ...
    if (!$error && $usuario_input && $pass_input) {
        try {
            ...
            // Buscar admin en tabla de admins (ver nota abajo)
            // Si no tienes tabla de admins, puedes usar credenciales hardcoded
            // con hash: ver comentario al final del archivo.
            $stmt = $pdo->prepare(
                "SELECT id, nombre, password_hash FROM admins WHERE usuario = :u LIMIT 1"
            );
            $stmt->execute([':u' => $usuario_input]);
            $admin = $stmt->fetch();
            ...
            if ($admin && password_verify($pass_input, $admin['password_hash'])) {
                ...
                // Delay artificial para dificultar fuerza bruta
                sleep(1);
                $error = 'Usuario o contraseña incorrectos.';
            }
        } catch (PDOException $e) {
            $error = 'Error de conexión. Contacta al desarrollador.';
        }
    } elseif (!$error) {
...
<title>Admin Login — Polla Mundialista</title>
...
.input-wrap i{
  ...
  color:var(--text-m);font-size:.95rem;pointer-events:none;
  transition:color .2s;
}
.input-wrap input{
  width:100%;
...
.input-wrap:focus-within i{color:var(--accent-l);}

/* Botón toggle password */
.btn-eye{
  position:absolute;right:.75rem;top:50%;transform:translateY(-50%);
  background:none;border:none;color:var(--text-m);cursor:pointer;
  font-size:1rem;padding:.2rem;transition:color .2s;
}
.btn-eye:hover{color:var(--accent-l);}

/* Ajuste para el ícono a la derecha del input cuando hay btn-eye */
...
<div class="login-card">
  ...
  <div class="card-top">
    ...
    <p>Polla Mundialista — Acceso restringido</p>
  </div>
    ...
    <form method="POST" id="login-form" autocomplete="off" novalidate>
      ...
      <div class="field">
        ...
        <div class="input-wrap has-eye">
          ...
          <button type="button" class="btn-eye" id="btn-eye" title="Mostrar contraseña">
  ...
  <div class="card-footer">
    <p>¿Problemas para acceder? <a href="mailto:admin@pollaworldcup.com">Contacta al admin</a></p>
  </div>
 ...
 *  NOTA: TABLA admins
 * ============================================================
 *  El login busca en una tabla `admins` con esta estructura:
 *
 *  CREATE TABLE admins (
 ...
 *  );
 *
 *  Para crear el primer admin, ejecuta este script UNA VEZ
 *  en tu servidor (o por CLI):
 *
 *  <?php
 *  $hash = password_hash('tu_contraseña_segura', PASSWORD_DEFAULT);
 *  $pdo->prepare("INSERT INTO admins (nombre, usuario, password_hash)
 *                 VALUES ('Tu Nombre', 'tu_usuario', ?)")
 *      ->execute([$hash]);
 *
 *  ──────────────────────────────────────────────────────────
 *  ALTERNATIVA SIN TABLA (credenciales hardcoded):
 *  Si no quieres tabla de admins, reemplaza el bloque PDO
 *  del login por esto:
 *
 *  $ADMIN_USER = 'admin';
 *  $ADMIN_HASH = password_hash('mi_pass_segura', PASSWORD_DEFAULT);
 *  // ↑ Genera ese hash UNA VEZ y pégalo directamente aquí.
 *
 *  if ($usuario_input === $ADMIN_USER && password_verify($pass_input, $ADMIN_HASH)) {
 *      session_regenerate_id(true);
